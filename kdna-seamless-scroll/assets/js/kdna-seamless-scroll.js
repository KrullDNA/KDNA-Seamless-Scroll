...
(function () {
	...
	function applyBgRules(cssText, panel, source, hid) {
		...
		while ((match = ruleRe.exec(css)) !== null) {
			...
			var decls = [];
			var declRe = /(background-image|--[\w-]+)\s*:\s*([^;]*url\([^;]*\)[^;]*)/gi;
			var d;
	...
	function loadNext() {
		...
		fetch(url, { credentials: 'same-origin' })
			...
			.then(function (data) {
				...
				// Rebuild this project's MotionPage animations against the DOM we
				// just injected. The v3 SDK path is the reliable one; the legacy
				// script re-run only runs if the SDK is not present.
				if (!rebuildMotionPageSDK(doc, newPanel)) {
	...
	// Rebuild a loaded project's MotionPage (v3 SDK) animations against the DOM we
	// just injected. MotionPage's SDK exposes window.Motion.context(fn): running a
	// project's own init function inside a fresh context builds that project's
	// timelines and ScrollTriggers and binds them to the elements now on the page,
	// exactly as they would build on the project's own page load. We pull each
	// loaded project's Motion.context(...) init out of its fetched HTML, strip the
	// DOMContentLoaded wrapper (which never fires again after first load), and
	// replay it. Returns true when the SDK path handled the rebuild.
	function rebuildMotionPageSDK(doc, panel) {

		var MP = window.Motion;
		if (!MP || typeof MP.context !== 'function') {
			log('MotionPage SDK (window.Motion.context) not present, skipping SDK rebuild.');
			return false;
		}

		// The SDK init lives in the inline script MotionPage prints after its bundle.
		var fns = [];
		doc.querySelectorAll('script:not([src])').forEach(function (s) {
			var code = s.textContent || '';
			if (code.indexOf('Motion.context(') > -1) {
				extractContextFns(code).forEach(function (src) { fns.push(src); });
			}
		});

		if (!fns.length) {
			log('No MotionPage SDK init found in fetched project.');
			return false;
		}

		var built = 0;
		fns.forEach(function (src) {
			try {
				// src is the function passed to Motion.context(...). Build and run it
				// inside its own context so this panel's timelines are tracked as one
				// unit (and could be reverted later if the panel is ever removed).
				var fn = new Function('return (' + src + ');')();
				var ctx = MP.context(fn);
				panel._kdnaMpContexts = panel._kdnaMpContexts || [];
				panel._kdnaMpContexts.push(ctx);
				built++;
			} catch (e) {
				log('MotionPage SDK rebuild error:', e);
			}
		});
	...
	// Pull every Motion.context(<fn>) argument out of a blob of script text, using
	// balanced-parenthesis matching (and skipping string literals) so minified code
	// with nested calls and selectors containing brackets is handled correctly.
	function extractContextFns(code) {
		var out = [];
		var marker = 'Motion.context(';
		var from = 0;
		var at;
		while ((at = code.indexOf(marker, from)) !== -1) {
